        ...
        createPanelContent(container, coordinator, options = {}) {
            ...
            this.register('convertWorkflowBtn', 'click', async () => {
                try {
                    ...
                    if (!tauriInvoke) {
                        this.showToast('Tauri API 未就绪，无法保存工作流', 'error');
                        return;
                    }
                    ...
                    // 使用新的模板转换接口
                    const resp = await tauriInvoke('import-and-convert-workflow', { workflow: parsed, name: name });
            ...
            workflows.forEach(workflow => {
                ...
                const viewOrEdit = async () => {
                    try {
                        const tauriInvoke = window.__TAURI__?.core?.invoke || window.__TAURI__?.invoke;
                        if (!tauriInvoke) {
                            this.showToast('Tauri API 未就绪', 'error');
                            return;
                        }
                        const resp = await tauriInvoke('comfyui:read-workflow', { name: workflow.name });
                        if (resp?.success) {
                            this.switchTab('import');
                            const nameInput = this.getElement('workflowName');
                ...
                this.register(item.querySelector('.delete-workflow'), 'click', async () => {
                    try {
                        const confirmed = await window.customConfirm(
                            `确定要删除工作流 "${workflow.displayName || workflow.name}" 吗？`,
                            '⚠️ 删除工作流'
                        );
                        if (!confirmed) return;
                        const tauriInvoke = window.__TAURI__?.core?.invoke || window.__TAURI__?.invoke;
                        ...
                        const resp = await tauriInvoke('comfyui:delete-workflow', { name: workflow.name });
                        if (resp?.success) {
