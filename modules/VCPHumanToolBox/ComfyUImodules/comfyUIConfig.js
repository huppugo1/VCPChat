    ...
    class ComfyUIConfigManager {
        ...
        async createUI(container, options = {}) {
            try {
                ...
                // 订阅主进程工作流变更事件，事件驱动刷新
                // Tauri 版本暂不支持事件监听，使用轮询方式
                // if (window.tauriAPI?.on) {
                //     window.tauriAPI.on('comfyui:workflows-changed', () => {
        ...
        async loadAvailableWorkflows() {
            ...
            if (!window.__TAURI__?.core?.invoke) {
                this.uiManager.showToast('Tauri API未就绪，无法加载工作流列表', 'error');
                return;
            }
            ...
            try {
                ...
                const workflows = await invoke('get_comfyui_workflows');
        ...
        async populateWorkflowSelect() {
            ...
            try {
                // 检查 Tauri API 是否可用
                if (!window.__TAURI__?.core?.invoke) {
                    this.uiManager.showToast('Tauri API未就绪,无法加载工作流', 'error');
                    workflowSelect.innerHTML = '<option value="">Tauri API未就绪</option>';
                    return;
                }
        ...
        async saveConfig() {
            ...
            try {
                ...
                if (!tauriInvoke) {
                    this.uiManager.showToast('Tauri API 未就绪，无法保存配置', 'error');
                ...
                const resp = await tauriInvoke('comfyui:save-config', { config: data });
                if (resp.success) {
                ...
                console.error('保存 ComfyUI 配置失败:', error);
        ...
        async loadConfig() {
            try {
                const tauriInvoke = window.__TAURI__?.core?.invoke || window.__TAURI__?.invoke;
                if (!tauriInvoke) {
                    this.uiManager.showToast('Tauri API 未就绪，回退至本地配置', 'warning');
                    await this.stateManager.loadConfig(); // Fallback to localStorage
                    return;
                }

                const resp = await tauriInvoke('comfyui:get-config', {});
                if (resp?.success && resp.data) {
                    this.stateManager.updateConfig(resp.data);
                    return;
                } else {
                     throw new Error(resp?.error || '获取配置失败');
                }
            } catch (e) {
                console.error('[ComfyUI] 获取配置失败，回退至本地存储', e);
