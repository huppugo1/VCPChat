...
async function callBackend(cmd, args) {
    ...
    if (!tauriInvoke) {
        console.error('[VCPHumanToolBox] Tauri API 不可用');
        return null;
    }
    try {
        return await tauriInvoke(cmd, args);
    } catch (e) {
        console.error(`[VCPHumanToolBox] 调用 ${cmd} 失败:`, e);
        return null;
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadSettings() {
        try {
            settings = await callBackend('vcp-ht-get-settings', {});
    ...
    async function saveSettings() {
        try {
            const result = await callBackend('vcp-ht-save-settings', { settings });
            if (!result || !result.success) {
                throw new Error(result?.error || '保存失败');
            }
            //console.log('[VCPHumanToolBox] 设置保存成功');
        } catch (error) {
            console.error('[VCPHumanToolBox] 保存设置失败:', error);
            throw error;
        }
    }
    ...
    // 全部清空功能
    async function clearAllFormData(toolName) {
        const confirmed = await window.customConfirm(
            '确定要清空所有内容吗？包括提示词、翻译内容、图片和额外图片。',
            '⚠️ 清空所有内容'
        );
        ...
        saveBtn.addEventListener('click', async () => {
            ...
            if (newLength >= 50 && newLength <= 1000) {
                ...
                try {
                    ...
                    console.error('保存设置失败：' + saveError.message); // 调试信息
                }
            } else {
                console.error('请输入 50-1000 之间的数值'); // 调试信息
            }
        });
    ...
    async function executeTool(toolName) {
        ...
        try {
            const result = await callBackend('vcp-ht-execute-tool-proxy', {
                url: VCP_SERVER_URL,
                apiKey: VCP_API_KEY,
            ...
            if (result && result.success) {
                ...
                renderResult({ status: 'error', error: result?.error || '执行失败' }, toolName);
            }
        } catch (error) {
    ...
    async function loadAndProcessWallpaper() {
        ...
        if (wallpaperUrl && wallpaperUrl !== 'none') {
            ...
            if (match && match[1]) {
                ...
                try {
                    const processedImageBase64 = await callBackend('vcp-ht-process-wallpaper', { path: imagePath });
    ...
    function initializeUI() {
        ...
        document.getElementById('minimize-btn').addEventListener('click', async () => {
            try {
                ...
                console.error('minimize_window invoke failed', e);
            }
        });
        document.getElementById('maximize-btn').addEventListener('click', async () => {
            try {
                ...
                console.error('toggle_maximize_window invoke failed', e);
            }
        });
        document.getElementById('close-btn').addEventListener('click', async () => {
            try {
                ...
                console.error('close_window invoke failed', e);
            }
        });
    ...
    async function openWorkflowEditor() {
        try {
            ...
            console.error(`打开工作流编排器失败: ${error.message}`); // 调试信息
        }
    }
