document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function callBackend(cmd, args) {
        ...
        if (!tauriInvoke) {
            console.error('[Notes] Tauri API 不可用');
            return null;
        }
        try {
            return await tauriInvoke(cmd, args);
        } catch (e) {
            console.error(`[Notes] 调用 ${cmd} 失败:`, e);
            return null;
        }
    }
    ...
    function makeImagesClickable() {
        previewContentDiv.querySelectorAll('img').forEach(img => {
            ...
            img.addEventListener('click', (e) => {
                ...
                (async () => {
                    try {
                        await callBackend('open_image_in_new_window', { url: imageUrl, title: imageTitle });
                    } catch (err) {
    ...
    async function loadNoteTree() {
        try {
            const result = await callBackend('read_notes_tree', {});
    ...
    async function createNewItem(type, ext = '.md') { // Default to .md for backward compatibility if needed
        ...
        if (activeItem) {
            if (activeItem.type === 'folder') {
                ...
                const parts = activeItem.path.replace(/\\/g, '/').split('/');
                parts.pop();
                parentPath = parts.join('/');
            }
        } else {
            // No active item, create at root.
            parentPath = await callBackend('get_notes_root_dir', {});
        }

        if (type === 'folder') {
            const folderName = '新建文件夹';
            await callBackend('create_note_folder', { parentPath, folderName });
        } else {
            const newNote = {
                title: '无标题笔记',
                content: '',
            ...
            const result = await callBackend('write_txt_note', { note: newNote });
    ...
    async function saveCurrentNote(isAutoSave = false) {
        ...
        let result;
        // 使用简单的字符串处理获取扩展名
        const extension = noteInTree.path.substring(noteInTree.path.lastIndexOf('.'));

        if (titleChanged) {
            // If title changes, we must use rename-item to change filename and content
            result = await callBackend('rename_item', {
                oldPath: noteInTree.path,
                newName: newTitle,
                newContentBody: newContent,
                ext: extension
            });
            if (result && result.success && result.newId) {
            ...
            result = await callBackend('write_txt_note', { note: noteData });
        }
    ...
    async function handleListDragStart(e) {
        ...
        if (window.tauriAPI && window.tauriAPI.getSelectionListenerStatus) {
            window.tauriAPI.getSelectionListenerStatus().then(isActive => {
                wasSelectionListenerActive = isActive;
                if (isActive && window.tauriAPI.toggleSelectionListener) {
                    window.tauriAPI.toggleSelectionListener(false);
                }
            }).catch(err => {
...
async function handleListDrop(e) {
    ...
    try {
        const result = await callBackend('notes_move_items', { sourcePaths, target }, () => {
            return (window.tauriAPI && typeof window.tauriAPI['notes:move-items'] === 'function') ? window.tauriAPI['notes:move-items']({ sourcePaths, target }) : null;
        });
...
function handleListDragEnd(e) {
    ...
    // Re-enable global selection listener only if it was active before the drag.
        if (window.tauriAPI && window.tauriAPI.toggleSelectionListener) {
            if (wasSelectionListenerActive) {
                callBackend('toggle_selection_listener', { enabled: true }, () => {
                    return window.tauriAPI.toggleSelectionListener(true);
                });
            }
    ...
    function handleItemContextMenu(e, item) {
        ...
        copyNoteBtn.onclick = async () => {
            const result = await callBackend('copy_note_content', { path: item.path }, () => {
                return (window.tauriAPI && typeof window.tauriAPI.copyNoteContent === 'function') ? window.tauriAPI.copyNoteContent(item.path) : Promise.resolve({ success: false });
            });
            if (result && result.success) {
    ...
    function startInlineRename(itemId) {
        ...
        const handleBlur = async () => {
            ...
            if (newName && newName !== currentName) {
                ...
                await window.tauriAPI.renameItem({ oldPath: item.path, newName: newName, ext: extension });
                await loadNoteTree();
            }
        };
    ...
    async function handleDirectDelete(isFromContextMenu = true) {
        ...
        if (confirmed) {
            for (const item of itemsToDelete) {
                const result = await callBackend('delete_item', { path: item.path }, () => {
                    return (window.tauriAPI && typeof window.tauriAPI.deleteItem === 'function') ? window.tauriAPI.deleteItem(item.path) : Promise.resolve({ success: false });
                });
                if (result.success) {
    ...
    async function initializeApp() {
        ...
        try {
            ...
            NOTES_DIR_CACHE = await callBackend('get_notes_root_dir', {}, () => {
                return (window.tauriAPI && typeof window.tauriAPI.getNotesRootDir === 'function') ? window.tauriAPI.getNotesRootDir() : null;
            });
        } catch (error) {
        ...
        const localResult = await callBackend('read_notes_tree', {}, () => {
            return (window.tauriAPI && typeof window.tauriAPI.readNotesTree === 'function') ? window.tauriAPI.readNotesTree() : null;
        });
        ...
        const cachedNetworkNotes = await window.tauriAPI.getCachedNetworkNotes();
        ...
        window.tauriAPI.scanNetworkNotes();

        // 5. Listen for the updated network notes to be returned
        window.tauriAPI.onNetworkNotesScanned((freshNetworkTree) => {
            // Ensure it's always an array, handling both old object format and null/undefined
        ...
        window.tauriAPI.onSharedNoteData(async (data) => {
            ...
            const newNoteData = {
                ...
                username: currentUsername,
                timestamp: Date.now(),
                directoryPath: await window.tauriAPI.getNotesRootDir() // Create in root by default
            };

            const result = await window.tauriAPI.writeTxtNote(newNoteData);
            if (result.success) {
                await loadNoteTree();
        ...
        if (window.tauriAPI.sendNotesWindowReady) {
            window.tauriAPI.sendNotesWindowReady();
        }
    }
        ...
        for (const item of items) {
            if (item.type.indexOf('image') !== -1) {
                ...
                reader.onload = async (e) => {
                    ...
                    const result = await window.tauriAPI.savePastedImageToFile({ data: base64Data, extension }, activeNoteId);
