...
Box.init().then(() => {
    ...
    if (window.tauriAPI && window.tauriAPI.getCurrentTheme) {
        window.tauriAPI.getCurrentTheme().then(theme => {
            applyTheme(theme);
            // Now that the theme is set, we can signal readiness.
            if (window.tauriAPI.sendDiceModuleReady) {
                window.tauriAPI.sendDiceModuleReady();
            }
        });
    } else {
        // Fallback if getCurrentTheme is not available, signal ready immediately
        if (window.tauriAPI && window.tauriAPI.sendDiceModuleReady) {
            window.tauriAPI.sendDiceModuleReady();
        }
    }
    ...
    // 监听来自主进程的AI投掷请求
    if (window.tauriAPI && window.tauriAPI.onRollDice) {
        window.tauriAPI.onRollDice((notation, options) => {
    ...
    Box.onRollComplete = (results) => {
        ...
        if (window.tauriAPI && window.tauriAPI.sendDiceRollComplete) {
            window.tauriAPI.sendDiceRollComplete(results);
        }
    };
