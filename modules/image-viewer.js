document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.__TAURI__ && window.__TAURI__.event && typeof window.__TAURI__.event.listen === 'function') {
        try {
            window.__TAURI__.event.listen('theme-updated', (event) => {
                try { applyTheme(event.payload); } catch (e) { console.error('applyTheme failed on theme-updated event', e); }
            });
        } catch (e) {
            console.warn('Failed to listen to theme-updated via Tauri event API, falling back to tauriAPI if available', e);
    ...
    clearBtn.addEventListener('click', async () => {
        const confirmed = await window.customConfirm('确定要清除所有绘图吗？', '⚠️ 清除画布');
        if (confirmed) {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            saveToHistory();
        }
