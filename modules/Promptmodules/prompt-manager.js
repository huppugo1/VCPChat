...
class PromptManager {
    ...
    async init(options) {
        ...
        // 从配置中读取当前模式，默认使用"原始富文本"
        // 如果 promptMode 不存在、为空字符串、null 或 undefined，都使用 'original'
        const promptMode = (this.config && this.config.promptMode) ? this.config.promptMode : null;
        this.currentMode = (promptMode && promptMode !== '' && promptMode !== null && promptMode !== undefined) ? promptMode : 'original';
    ...
    async loadCustomModeNames() {
        try {
            ...
            if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                try {
                    settings = await window.__TAURI__.invoke('load_settings');
                } catch (e) {
                    console.warn('load_settings invoke failed, falling back to tauriAPI.loadSettings', e);
                    settings = this.tauriAPI ? await this.tauriAPI.loadSettings() : null;
                }
            } else if (this.tauriAPI && typeof this.tauriAPI.loadSettings === 'function') {
                settings = await this.tauriAPI.loadSettings();
            }
    ...
    async saveCustomModeNames() {
        try {
            // Prefer Tauri invoke for settings, fallback to tauriAPI
            let settings = null;
            if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                try {
                    settings = await window.__TAURI__.invoke('load_settings');
            ...
            if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                try {
                    await window.__TAURI__.invoke('save_settings', newSettings);
                } catch (e) {
                    console.warn('save_settings invoke failed, falling back to tauriAPI.saveSettings', e);
                    if (this.tauriAPI && typeof this.tauriAPI.saveSettings === 'function') {
                        await this.tauriAPI.saveSettings(newSettings);
                    }
                }
            } else if (this.tauriAPI && typeof this.tauriAPI.saveSettings === 'function') {
                await this.tauriAPI.saveSettings(newSettings);
            }
        } catch (error) {
    ...
    async switchMode(mode) {
        ...
        // 注意：不在这里保存到配置文件，完整保存应该在用户点击"保存"按钮时进行
    }
    ...
    getAllConfigData() {
        ...
        // 获取各个模块的配置数据
        if (this.originalModule && typeof this.originalModule.getConfigData === 'function') {
            Object.assign(configData, this.originalModule.getConfigData());
        }
        
        if (this.modularModule && typeof this.modularModule.getConfigData === 'function') {
            Object.assign(configData, this.modularModule.getConfigData());
        }
        
        if (this.presetModule && typeof this.presetModule.getConfigData === 'function') {
            Object.assign(configData, this.presetModule.getConfigData());
        }
