...
class ModularPromptModule {
    ...
    async loadData() {
        ...
        // 2. [新增] 加载全局仓库数据
        try {
            const response = await this.callBackend('get_global_warehouse', {}, () => {
                if (this.tauriAPI && typeof this.tauriAPI.getGlobalWarehouse === 'function') {
                    return this.tauriAPI.getGlobalWarehouse();
                }
            ...
            if (response && response.success) {
                ...
                if (response && response.error) console.error('Failed to load global warehouse:', response.error);
     ...
     */
    async callBackend(cmd, args, fallback) {
        try {
            ...
            console.error(`[ModularPromptModule callBackend] ${cmd} failed:`, err);
            if (typeof fallback === 'function') {
                try { return await fallback(); } catch (e) { /* ignore */ }
            }
            throw err;
        }
    }
    ...
    createToolbar() {
        ...
        addNewlineBtn.type = 'button'; // 防止触发表单提交
        ...
        addNewlineBtn.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.addBlock('newline');
        };
    ...
    editBlock(block, index) {
        ...
        block.variants.forEach((variant, idx) => {
            dialogHTML += `
                        <div class="variant-item-edit" data-index="${idx}">
                            ...
                            ${block.variants.length > 1 ? `<button type="button" class="remove-variant-btn" data-index="${idx}">×</button>` : ''}
                        </div>`;
        });

        dialogHTML += `
                    </div>
                    <button type="button" class="add-variant-btn">+ 添加内容条目</button>
                </div>
                <div class="dialog-buttons">
                    <button type="button" class="dialog-btn dialog-btn-cancel">取消</button>
                    <button type="button" class="dialog-btn dialog-btn-save">保存</button>
                </div>
            </div>
        `;
        ...
        addVariantBtn.onclick = (e) => {
            ...
            variantItem.innerHTML = `
                ...
                <button type="button" class="remove-variant-btn" data-index="${newIndex}">×</button>
            `;
    ...
    renderWarehouse() {
        ...
        addWarehouseBtn.type = 'button'; // 防止触发表单提交
    ...
    /**
     * [修改后] 保存数据（仅更新内存，不写入文件）
     * 全局仓库可以实时保存，但 agent 配置只在点击保存按钮时保存
     */
    async save() {
        // 1. [保留] 提取全局仓库数据并独立保存（全局数据可以实时保存）
        const globalBlocksToSave = this.hiddenBlocks['global'] || [];
        try {
            await this.callBackend('save_global_warehouse', { data: globalBlocksToSave }, () => {
                if (this.tauriAPI && typeof this.tauriAPI.saveGlobalWarehouse === 'function') {
                    return this.tauriAPI.saveGlobalWarehouse(globalBlocksToSave);
                }
                return Promise.reject(new Error('No backend for save_global_warehouse'));
            });
        } catch (error) {
            console.error('Error saving global warehouse:', error);
        }

        // 2. [修改] 仅更新内存缓存，不保存到 agent 配置文件
        // agent 配置（blocks, hiddenBlocks 等）将在用户点击"保存"按钮时统一保存
    ...
    createWarehouse() {
        ...
        dialog.innerHTML = `
            ...
            <div class="dialog-content">
                ...
                <div class="dialog-buttons">
                    <button type="button" class="dialog-btn dialog-btn-cancel">取消</button>
                    <button type="button" class="dialog-btn dialog-btn-save">创建</button>
                </div>
            </div>
        `;
        ...
        const createAction = () => {
            ...
            if (name === 'default') {
                //console.log('不能使用 "default" 作为仓库名称');
                return;
            }
    ...
    renameWarehouse(oldName) {
        ...
        dialog.innerHTML = `
            ...
            <div class="dialog-content">
                ...
                <div class="dialog-buttons">
                    <button type="button" class="dialog-btn dialog-btn-cancel">取消</button>
                    <button type="button" class="dialog-btn dialog-btn-save">确定</button>
                </div>
            </div>
        `;
     ...
     * 删除仓库
     */
    async deleteWarehouse(warehouseName) {
        const confirmed = await window.customConfirm(
            `确定要删除仓库 "${warehouseName}" 吗？其中的积木块也会被删除。`,
            '⚠️ 删除仓库'
        );
        if (!confirmed) {
            return;
