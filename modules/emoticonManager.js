    ...
    async function loadUserEmoticons() {
        try {
            ...
            if (!window.__TAURI__?.invoke) {
                console.error('[EmoticonManager] Tauri API 不可用');
                userEmoticons = [];
                return;
            }
            ...
            if (!userName) {
                console.warn('[EmoticonManager] 设置中未找到用户名');
                userEmoticons = [];
                return;
            }

            const emoticonLibrary = await window.__TAURI__.invoke('get_emoticon_library');
            if (!emoticonLibrary || !Array.isArray(emoticonLibrary)) {
                 console.error('[EmoticonManager] 加载或解析 emoticon_library.json 失败');
                 userEmoticons = [];
                 return;
            }
            ...
            //console.log(`[EmoticonManager] 为用户 "${userName}" 加载了 ${userEmoticons.length} 个表情`);

        } catch (error) {
            console.error('[EmoticonManager] 加载用户表情时出错:', error);
            userEmoticons = [];
        }
    }
