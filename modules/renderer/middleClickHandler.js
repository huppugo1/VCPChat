...
async function handleMiddleClickQuickAction(event, messageItem, message, quickAction) {
    const { tauriAPI, uiHelper } = mainRendererReferences;
    ...
    switch (quickAction) {
                        ...
                        if (!currentContent || currentContent.trim() === '') {
                            ...
                            try {
                                ...
                                if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                                    try {
                                        result = await window.__TAURI__.invoke('get_original_message_content', {
                                            id: currentSelectedItemVal.id,
                                            type: currentSelectedItemVal.type,
                                            topicId: currentTopicIdVal,
                                            messageId: message.id
                                        });
                                    } catch (e) {
                                        console.warn('get_original_message_content invoke failed, falling back to tauriAPI.getOriginalMessageContent', e);
                                        result = tauriAPI ? await tauriAPI.getOriginalMessageContent(currentSelectedItemVal.id, currentSelectedItemVal.type, currentTopicIdVal, message.id) : null;
                                    }
                                } else if (tauriAPI && typeof tauriAPI.getOriginalMessageContent === 'function') {
                                    result = await tauriAPI.getOriginalMessageContent(currentSelectedItemVal.id, currentSelectedItemVal.type, currentTopicIdVal, message.id);
                                }
        ...
        case 'readAloud':
            ...
            if (message.role === 'assistant') {
                ...
                // Prefer Tauri invoke to get agent config, then invoke TTS; fallback to tauriAPI
                (async () => {
                    try {
                        let agentCfg = null;
                        if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                            try {
                                agentCfg = await window.__TAURI__.invoke('get_agent_config', { agentId: agentId });
                            } catch (e) {
                                console.warn('get_agent_config invoke failed, falling back to tauriAPI.getAgentConfig', e);
                                agentCfg = tauriAPI ? await tauriAPI.getAgentConfig(agentId) : null;
                            }
                        } else {
                            agentCfg = tauriAPI ? await tauriAPI.getAgentConfig(agentId) : null;
                        }

                        if (agentCfg && agentCfg.ttsVoicePrimary) {
                            const contentDiv = messageItem.querySelector('.md-content');
                            let textToRead = '';
                            if (contentDiv) {
                                const contentClone = contentDiv.cloneNode(true);
                            ...
                            if (textToRead.trim()) {
                                const speakPayload = {
                                    text: textToRead,
                                    voice: agentCfg.ttsVoicePrimary,
                                    speed: agentCfg.ttsSpeed || 1.0,
                                    msgId: message.id,
                                    ttsRegex: agentCfg.ttsRegexPrimary,
                                    voiceSecondary: agentCfg.ttsVoiceSecondary,
                                    ttsRegexSecondary: agentCfg.ttsRegexSecondary
                                };
                                if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                                    window.__TAURI__.invoke('sovits_speak', speakPayload).catch(err => {
                                        console.warn('sovits_speak invoke failed, falling back to tauriAPI.sovitsSpeak', err);
                                        if (tauriAPI && typeof tauriAPI.sovitsSpeak === 'function') {
                                            tauriAPI.sovitsSpeak(speakPayload);
                                        }
                                    });
                                } else if (tauriAPI && typeof tauriAPI.sovitsSpeak === 'function') {
                                    tauriAPI.sovitsSpeak(speakPayload);
                                }
                            } else {
            ...
            (async () => {
                try {
                    ...
                    if (!contentString || contentString.trim() === '') {
                        uiHelper.showToastNotification("消息内容为空，无法打开阅读模式", "warning");
                        return;
                    }
                    ...
                    if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                        await window.__TAURI__.invoke('open_text_in_new_window', { 
                            content: contentString, 
                            title: windowTitle, 
                            theme: currentTheme 
                        });
                    } else if (tauriAPI && typeof tauriAPI.openTextInNewWindow === 'function') {
                        await tauriAPI.openTextInNewWindow(contentString, windowTitle, currentTheme);
                    } else {
                        uiHelper.showToastNotification("阅读模式功能不可用", "error");
                    }
                } catch (error) {
                    console.error('[MiddleClick ReadMode] 打开阅读窗口失败:', error);
                    uiHelper.showToastNotification("打开阅读窗口失败: " + error.message, "error");
                }
            })();
            break;
        ...
        case 'regenerate':
            ...
            if (message.role === 'assistant') {
                ...
                if (enableConfirmation) {
                    ...
                    // 如果不是最后一条消息，显示警告对话框
                    if (!isLastMessage) {
                        const confirmRegenerate = await window.customConfirm(
                ...
                if (message.isGroupMessage) {
                    ...
                    if (currentSelectedItem.type === 'group' && currentTopicId && message.id && message.agentId) {
                        // 调用群聊重新回复的IPC接口
                        if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                            window.__TAURI__.invoke('redo_group_chat_message', { groupId: currentSelectedItem.id, topicId: currentTopicId, messageId: message.id, agentId: message.agentId })
                                .then(() => {
                                    ...
                                    if (mainRendererReferences.tauriAPI && mainRendererReferences.tauriAPI.redoGroupChatMessage) {
                                        mainRendererReferences.tauriAPI.redoGroupChatMessage(currentSelectedItem.id, currentTopicId, message.id, message.agentId);
                                        uiHelper.showToastNotification("已开始重新生成回复", "success");
                                    } else {
                                        uiHelper.showToastNotification("群聊重新回复功能暂时不可用", "warning");
                                    }
                                });
                        } else if (mainRendererReferences.tauriAPI && mainRendererReferences.tauriAPI.redoGroupChatMessage) {
                            mainRendererReferences.tauriAPI.redoGroupChatMessage(currentSelectedItem.id, currentTopicId, message.id, message.agentId);
                            uiHelper.showToastNotification("已开始重新生成回复", "success");
                        } else {
                            uiHelper.showToastNotification("群聊重新回复功能暂时不可用", "warning");
...
function updateMiddleClickQuickAction(newAction) {
    ...
    // Save settings
    // Prefer Tauri invoke to save settings, fallback to tauriAPI.saveSettings
    if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
        window.__TAURI__.invoke('save_settings', {
            ...globalSettings,
            middleClickQuickAction: newAction
        }).then(result => {
            ...
            console.warn('save_settings invoke failed, falling back to tauriAPI.saveSettings', err);
            if (mainRendererReferences.tauriAPI && mainRendererReferences.tauriAPI.saveSettings) {
                mainRendererReferences.tauriAPI.saveSettings({
                    ...globalSettings,
                    middleClickQuickAction: newAction
                }).then(result => {
                    if (result.success) {
    ...
    } else if (mainRendererReferences.tauriAPI && mainRendererReferences.tauriAPI.saveSettings) {
        mainRendererReferences.tauriAPI.saveSettings({
            ...globalSettings,
            middleClickQuickAction: newAction
        }).then(result => {
