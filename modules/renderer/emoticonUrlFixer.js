        ...
        tauriAPI = api;
            try {
                ...
                if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                    try {
                        library = await window.__TAURI__.invoke('get_emoticon_library');
                    } catch (e) {
                        console.warn('get_emoticon_library invoke failed, falling back to tauriAPI.getEmoticonLibrary', e);
                        library = tauriAPI && typeof tauriAPI.getEmoticonLibrary === 'function' ? await tauriAPI.getEmoticonLibrary() : null;
                    }
                } else if (tauriAPI && typeof tauriAPI.getEmoticonLibrary === 'function') {
                    library = await tauriAPI.getEmoticonLibrary();
                }
                ...
                if (library && library.length > 0) {
                    ...
                    //console.log('[EmoticonFixer] No emoticon library found, emoticon fixing disabled.');
