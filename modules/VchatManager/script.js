document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            for (const item of combinedItemOrder) {
                ...
                if (configStr) {
                    ...
                    // Check if avatar file exists, otherwise use default (use relative path, resolved later via avatarManager)
                    const avatarExists = await window.api.readFile(avatarPath);
                    const finalAvatarPath = avatarExists ? avatarPath :
                    (item.type === 'agent' ? 'AppData/assets/default_avatar.png' : 'AppData/assets/default_group_avatar.png');
    ...
    function renderSidebar(data) {
        ...
        settings.combinedItemOrder.forEach(item => {
            ...
            // Create avatar img element and resolve URL via avatarManager
            const avatarImg = document.createElement('img');
            avatarImg.classList.add('avatar');
            avatarImg.alt = `${itemData.name || 'Unknown'} avatar`;
            // Resolve path (may be AppData relative or assets)
            (async () => {
                try {
                    const raw = itemData.avatar || (item.type === 'agent' ? 'AppData/assets/default_avatar.png' : 'AppData/assets/default_group_avatar.png');
                    // avatarManager.resolveAvatarUrl can handle relative AppData paths and add cache busters
                    const resolved = await window.avatarManager.resolveAvatarUrl(raw);
                    avatarImg.src = resolved || `../${raw}`;
                } catch (e) {
                    // Fallback to direct assignment if resolve fails
                    avatarImg.src = itemData.avatar ? `../${itemData.avatar}` : `../AppData/assets/default_avatar.png`;
                }
            })();
            // onerror fallback uses avatarManager to resolve default
            avatarImg.onerror = async () => {
                try {
                    const def = item.type === 'agent' ? 'AppData/assets/default_avatar.png' : 'AppData/assets/default_group_avatar.png';
                    const resolvedDef = await window.avatarManager.resolveAvatarUrl(def);
                    avatarImg.src = resolvedDef || `../${def}`;
                } catch (e) {
                    avatarImg.src = item.type === 'agent' ? '../assets/default_avatar.png' : '../assets/default_group_avatar.png';
                }
            };

            const nameSpan = document.createElement('span');
            nameSpan.textContent = itemData.name || 'Unknown Agent';
    ...
    async function saveEdit(contentDiv, newText) {
        ...
        if (result.success) {
            ...
            console.error(`Error saving file: ${result.error}`); // 调试信息
        }
    }
   ...
   function renderJsonEditor(history) {
    ...
    async function navigateToMessage(message) {
        ...
        if (!sidebarItem) {
            ...
            console.error('Error: Could not find the corresponding Agent/Group in the sidebar.'); // 调试信息
            return;
        }
    ...
    async function applyConsistencyFixes() {
        ...
        if (!lastCheckResults || lastCheckResults.totalIssues === 0) {
            //console.log('No issues to fix.'); // 调试信息
            return;
        }
        ...
        if (selectedIssues.length === 0) {
            console.error('Please select at least one issue to fix.'); // 调试信息
            return;
        }
        ...
        try {
            ...
            console.error(`Error applying fixes: ${error.message}`); // 调试信息
        } finally {
