...
export async function handleSaveGlobalSettings(e, deps) {
        ...
        try {
            ...
            async function callBackend(cmd, args) {
                if (!window.__TAURI__?.invoke) {
                    console.error('[GlobalSettings] Tauri API 不可用');
                    throw new Error('Tauri API 不可用');
                }
                try {
                    return await window.__TAURI__.invoke(cmd, args);
                } catch (err) {
                    console.error(`[GlobalSettings] 调用 ${cmd} 失败:`, err);
                    throw err;
                }
            }

            const avatarSaveResult = await callBackend('save_user_avatar', {
                name: userAvatarCropped.name,
                type: userAvatarCropped.type,
                buffer: arrayBuffer
            });
            if (avatarSaveResult.success) {
                // 使用 avatarManager 解析头像 URL
                let resolvedAvatarUrl;
                try {
                    // 通过 avatarManager 解析路径并添加缓存破坏参数
                    resolvedAvatarUrl = await window.avatarManager.resolveAvatarUrl(avatarSaveResult.avatarUrl);
                    // 添加时间戳防止浏览器缓存
                    const separator = resolvedAvatarUrl.includes('?') ? '&' : '?';
                    resolvedAvatarUrl = `${resolvedAvatarUrl}${separator}t=${Date.now()}`;
                } catch (e) {
                    console.warn('[GlobalSettings] Failed to resolve avatar URL:', e);
                    resolvedAvatarUrl = avatarSaveResult.avatarUrl;
                }
                
                refs.globalSettings.get().userAvatarUrl = resolvedAvatarUrl;
                // 统一使用 avatarManager 更新 UI（会同时更新消息区与预览，并通知 messageRenderer）
                try {
                    // 直接使用统一的 avatarManager 更新 UI（无回退）
                    await window.avatarManager.applyAvatarToContext('user', null, avatarSaveResult.avatarUrl);
                } catch (e) {
                    console.warn('avatarManager.applyAvatarToContext failed:', e);
                }
                if (avatarSaveResult.needsColorExtraction) {
                    if (window.getDominantAvatarColor) {
                        window.getDominantAvatarColor(avatarSaveResult.avatarUrl).then(async avgColor => {
                            if (avgColor) {
                                try {
                                    const saveColorResult = await callBackend('save_avatar_color', { type: 'user', id: 'user_global', color: avgColor });
                                    if (saveColorResult && saveColorResult.success) {
                                        ...
                                        console.warn("[GlobalSettings] 保存用户头像颜色失败:", saveColorResult?.error);
                                    }
                                } catch (err) {
                                    console.error("[GlobalSettings] 保存用户头像颜色出错:", err);
                                }
                            }
                        });
                    }
    ...
    const result = await callBackend('save_settings', { settings: newSettings });
    if (result.success) {
        ...
        if (refs.globalSettings.get().vcpLogUrl && refs.globalSettings.get().vcpLogKey) {
             try {
                 await callBackend('connect_vcp_log', { url: refs.globalSettings.get().vcpLogUrl, key: refs.globalSettings.get().vcpLogKey });
             } catch (err) {
                 console.warn('[GlobalSettings] 连接 VCP Log 失败:', err);
             }
        } else {
             try {
                 await callBackend('disconnect_vcp_log', {});
             } catch (err) {
                 console.warn('[GlobalSettings] 断开 VCP Log 失败:', err);
             }
