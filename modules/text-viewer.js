...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fixEmoticonImagesInContainer(container) {
        ...
        if (window.tauriAPI) {
            await emoticonFixer.initialize(window.tauriAPI);
        }
...
`;
    ...
    if (window.__TAURI__ && window.__TAURI__.event && typeof window.__TAURI__.event.listen === 'function') {
        try {
            window.__TAURI__.event.listen('theme-updated', (evt) => {
                try {
                    // Tauri event payload typically contains the new theme
                    applyTheme(evt && evt.payload ? evt.payload : undefined);
                } catch (e) {
                    console.error('[TextViewer] Error applying theme from Tauri event:', e);
                }
            });
        } catch (e) {
            console.error('[TextViewer] Failed to listen theme-updated via Tauri:', e);
            if (window.tauriAPI && window.tauriAPI.onThemeUpdated) {
                window.tauriAPI.onThemeUpdated(applyTheme);
            } else {
                //console.log('[TextViewer] tauriAPI.onThemeUpdated not found. Theme updates will not be received.');
            }
        }
    } else if (window.tauriAPI && window.tauriAPI.onThemeUpdated) {
        window.tauriAPI.onThemeUpdated(applyTheme);
    } else {
        //console.log('[TextViewer] No theme update mechanism found. Theme updates will not be received.');
    }
    ...
    async function py_penetration_executor(code, outputContainer) {
        ...
        try {
            const backendResult = await callBackend(
                ...
                { code },
                () => {
                    if (window.tauriAPI && typeof window.tauriAPI.executePythonCode === 'function') {
                        return window.tauriAPI.executePythonCode(code);
                    }
                    return Promise.reject(new Error('No backend available for execute_python_code'));
                }
            );

            // backendResult may be an object { stdout, stderr } or a raw string; normalize
            const stdout = (backendResult && backendResult.stdout) ? backendResult.stdout : (typeof backendResult === 'string' ? backendResult : '');
            const stderr = (backendResult && backendResult.stderr) ? backendResult.stderr : '';

            //console.log('[text-viewer] Backend execution returned.');
            ...
            console.error("[text-viewer] Local Python execution error (py_penetration_executor):", error);
    ...
    if (shareToNotesButton && contentDiv) {
        shareToNotesButton.addEventListener('click', async () => {
            const noteTitle = document.title || '来自阅读模式的分享'; // 使用页面标题或默认标题
            try {
                await callBackend(
                    'open_notes_with_content',
                    { title: noteTitle, content: originalRawContent },
                    () => {
                        if (window.tauriAPI && typeof window.tauriAPI.openNotesWithContent === 'function') {
                            return window.tauriAPI.openNotesWithContent({
                                title: noteTitle,
                                content: originalRawContent,
                            });
                        }
                        return Promise.reject(new Error('No backend available for open_notes_with_content'));
                    }
                );
            } catch (err) {
                console.error('[text-viewer] Error sharing to notes:', err);
                console.error('分享到笔记失败，请检查控制台获取更多信息。'); // 调试信息
            }
        });
    }
...
${codeContent}
         ...
         contextMenuShareScreenshotButton.addEventListener('click', () => {
            ...
            if (window.html2canvas && mainContentDiv) {
                html2canvas(mainContentDiv, {
                    ...
                    if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                        window.__TAURI__.invoke('open_image_viewer', { src: imageDataUrl, title: `截图分享 - ${document.title}` }).catch(err => {
                            console.error('open_image_viewer failed', err);
                            console.error('截图功能不可用。'); // 调试信息
                        });
                    } else if (window.tauriAPI && window.tauriAPI.openImageViewer) {
                        window.tauriAPI.openImageViewer({
                            src: imageDataUrl,
                            title: `截图分享 - ${document.title}`
                        });
                    } else {
                        console.error('tauriAPI.openImageViewer is not available.');
                        console.error('截图功能不可用。'); // 调试信息
                    }
                }).catch(err => {
                    console.error('Error generating screenshot:', err);
                    console.error('生成截图失败。'); // 调试信息
                });
            }
        });
    ...
    if (minimizeBtn && maximizeBtn && closeBtn) {
        minimizeBtn.addEventListener('click', () => {
            if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                window.__TAURI__.invoke('minimize_window').catch(err => console.error('minimize_window failed', err));
            } else if (window.tauriAPI) {
                window.tauriAPI.minimizeWindow();
            }
        });

        maximizeBtn.addEventListener('click', () => {
            if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
                window.__TAURI__.invoke('toggle_maximize_window').catch(err => console.error('toggle_maximize_window failed', err));
            } else if (window.tauriAPI) {
                window.tauriAPI.maximizeWindow();
            }
        });
