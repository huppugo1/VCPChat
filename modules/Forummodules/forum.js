...
/**
 * Unified backend caller for forum module: prefer Tauri invoke, fallback to tauriAPI function.
 * @param {string} cmd
 * @param {object} args
...
async function callBackend(cmd, args, fallback) {
    if (window.__TAURI__ && typeof window.__TAURI__.invoke === 'function') {
        try {
            ...
            console.warn('[Forum] Tauri invoke failed for', cmd, e);
        }
    }
    if (window.tauriAPI && typeof fallback === 'function') {
        try { return await fallback(); } catch (e) { console.warn('[Forum] tauriAPI fallback failed for', cmd, e); }
    }
    return null;
}
...
// ========== Window Controls ==========
document.getElementById('minimize-forum-btn')?.addEventListener('click', () => {
...
document.getElementById('maximize-forum-btn')?.addEventListener('click', () => {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // Load settings via unified backend caller
        try {
        ...
        // Listen for live theme changes: prefer Tauri events
    ...
    document.body.addEventListener('click', (event) => {
        ...
        if (link && (link.protocol === 'http:' || link.protocol === 'https:')) {
            ...
            try {
                ...
                console.warn('open_external/openExternal failed, falling back to window.open', err);
                try { window.open(url, '_blank'); } catch (e) { /* ignore */ }
            }
        }
    });
...
async function loadForumConfig() {
    try {
        const config = await callBackend('load_forum_config', {}, () => {
            if (window.tauriAPI && typeof window.tauriAPI.loadForumConfig === 'function') {
                return window.tauriAPI.loadForumConfig();
            }
...
async function handleLogin() {
    ...
    try {
        ...
        try {
            await callBackend('save_forum_config', { config: forumConfig }, () => {
                if (window.tauriAPI && typeof window.tauriAPI.saveForumConfig === 'function') {
                    return window.tauriAPI.saveForumConfig(forumConfig);
                }
                return Promise.resolve();
            });
        } catch (err) {
            console.warn('save_forum_config/saveForumConfig failed', err);
        }
...
async function loadAgentsList() {
    try {
        let agentsData = null;
        agentsData = await callBackend('load_agents_list', {}, () => {
            return (window.tauriAPI && typeof window.tauriAPI.loadAgentsList === 'function') ? window.tauriAPI.loadAgentsList() : null;
        });
...
async function loadEmoticonLibrary() {
    try {
        let library = null;
        library = await callBackend('get_emoticon_library', {}, () => {
            return (window.tauriAPI && typeof window.tauriAPI.getEmoticonLibrary === 'function') ? window.tauriAPI.getEmoticonLibrary() : null;
        });
...
function setupImageViewer(container) {
    ...
    images.forEach(img => {
        ...
        img.addEventListener('click', (e) => {
            ...
            (async () => {
                try {
                    await callBackend('open_image_viewer', { src: img.src, title: '图片查看' }, () => {
                        if (window.tauriAPI && typeof window.tauriAPI.openImageViewer === 'function') {
                            return window.tauriAPI.openImageViewer({ src: img.src, title: '图片查看' });
                        }
                        return Promise.resolve();
                    });
                } catch (err) {
                    console.error('open_image_viewer/openImageViewer failed', err);
                }
            })();
        });
    });
...
async function getAvatarForUser(username) {
    ...
    try {
        ...
        if (isCurrentUser) {
            let userAvatar = null;
            userAvatar = await callBackend('load_user_avatar', {}, () => {
                return (window.tauriAPI && typeof window.tauriAPI.loadUserAvatar === 'function') ? window.tauriAPI.loadUserAvatar() : null;
            });
            if (userAvatar) {
        ...
        for (const agent of agentsList) {
            ...
            if (agentNameLower.includes(usernameLower) || usernameLower.includes(agentNameLower)) {
                ...
                agentAvatar = await callBackend('load_agent_avatar', { folder: agent.folder }, () => {
                    return (window.tauriAPI && typeof window.tauriAPI.loadAgentAvatar === 'function') ? window.tauriAPI.loadAgentAvatar(agent.folder) : null;
                });
                if (agentAvatar) {
...
async function saveSettings() {
    ...
    try {
        await callBackend('save_forum_config', { config: newConfig }, () => {
            if (window.tauriAPI && typeof window.tauriAPI.saveForumConfig === 'function') {
                return window.tauriAPI.saveForumConfig(newConfig);
            }
            return Promise.resolve();
        });
...
async function handleQuickReply(uid, container) {
    ...
    try {
        ...
        console.error('Reply failed: ' + error.message); // 调试信息
        btn.disabled = false;
        btn.textContent = '发送';
    }
}
