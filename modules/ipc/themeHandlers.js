...
function initialize(options) {
    ...
    ipcMain.handle('get-wallpaper-thumbnail', async (event, rawPath) => {
        ...
        // Try multiple possible paths for the wallpaper file
        const possiblePaths = [
            path.resolve(PROJECT_ROOT, 'Themesmodules', cleanedPath),  // Development mode
            path.resolve(PROJECT_ROOT, '..', '..', 'Themesmodules', cleanedPath),  // Production mode (relative to src-tauri/target/release)
            path.resolve(PROJECT_ROOT, 'assets', 'wallpaper', path.basename(cleanedPath)),  // Direct asset path
            path.resolve(PROJECT_ROOT, '..', '..', 'assets', 'wallpaper', path.basename(cleanedPath)),  // Production asset path
            path.resolve(PROJECT_ROOT, 'AppData', 'assets', 'wallpaper', path.basename(cleanedPath)), // AppData asset path (runtime)
        ];
        ...
        if (!absolutePath) {
            throw new Error(`Original wallpaper file not found at any of the possible paths: ${possiblePaths.join(', ')}`);
        }
        ...
        try {
            ...
            // If thumbnail generation fails, return the original file path as fallback
            return absolutePath;
        }
    });
}
