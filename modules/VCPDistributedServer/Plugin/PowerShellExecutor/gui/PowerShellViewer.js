...
function fitTerminal() {
    try {
        ...
        // 在调整前端后，立即将新的尺寸发送到后端
        if (window.tauriAPI) {
            window.tauriAPI.send('powershell-resize', { cols: term.cols, rows: term.rows });
        }
    } catch (e) {
...
function sendCommand() {
    ...
    if (command.trim() && window.tauriAPI) {
        term.write(command + '\r\n');
        window.tauriAPI.send('powershell-command', command);
        commandInput.value = '';
        commandInput.focus();
    }
}
...
if (window.tauriAPI) {
    ...
    window.tauriAPI.on('powershell-data', (data) => {
    ...
    window.tauriAPI.on('powershell-clear', () => {
        term.clear();
    });
    window.tauriAPI.on('theme-init', ({ themeName }) => {
        // 恢复主程序使用的标准主题切换逻辑：
    ...
    terminalContainer.addEventListener('contextmenu', (e) => {
        ...
        if (selection) {
            window.tauriAPI.send('copy-to-clipboard', selection);
        }
    });
    ...
    console.error('Fatal Error: tauriAPI not found.');
...
window.addEventListener('DOMContentLoaded', () => {
    fitTerminal();
    if (window.tauriAPI) {
        window.tauriAPI.send('powershell-gui-ready');
    }
    ...
    minimizeBtn.addEventListener('click', () => {
        if (window.tauriAPI) {
            try {
                window.tauriAPI.invoke('minimize_window');
            } catch (e) {
    ...
    maximizeBtn.addEventListener('click', () => {
        if (window.tauriAPI) {
            try {
                window.tauriAPI.invoke('toggle_maximize_window');
            } catch (e) {
                console.error('invoke toggle_maximize_window failed', e);
            }
        }
    });
    ...
    closeBtn.addEventListener('click', () => {
        if (window.tauriAPI) {
            try {
                window.tauriAPI.invoke('close_window');
            } catch (e) {
