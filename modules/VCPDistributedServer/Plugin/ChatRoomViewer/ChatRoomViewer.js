...
async function findAgentAvatar(agentPath) {
    ...
    // 如果没有找到头像，返回默认头像路径（使用可配置的静态服务器基址或回退到 localhost）
    const appDataServerBase = process.env.VCP_APPDATA_SERVER_BASE || 'http://127.0.0.1:1422';
    return `${appDataServerBase}/AppData/assets/default_avatar.png`;
}
