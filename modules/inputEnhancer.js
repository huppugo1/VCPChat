...
// Removed: let tauriAPI; - It will be assigned from refs and use the global window.tauriAPI
 ...
 * @param {object} refs.tauriAPI - The exposed electron API from preload.js.
...
function initializeInputEnhancer(refs) {
    if (!refs.messageInput || !refs.tauriAPI || !refs.attachedFiles || !refs.updateAttachmentPreview || !refs.getCurrentAgentId || !refs.getCurrentTopicId) {
        console.error('[InputEnhancer] Initialization failed: Missing required references.');
        return;
    }
    // Assign tauriAPI from refs to the module-scoped variable (which is no longer declared with let/const at the top)
    // This assumes tauriAPI is available in the scope where initializeInputEnhancer is called (e.g. window.tauriAPI)
    // and is correctly passed in through refs.
    const localtauriAPI = refs.tauriAPI; // Use a local const to avoid confusion if needed, or directly use refs.tauriAPI
    //console.log('[InputEnhancer] Initializing with localtauriAPI:', localtauriAPI); // Log the API object
    ...
    messageInput.addEventListener('drop', async (event) => {
        ...
        if (!agentId || !topicId) {
            console.error("请先选择一个Agent和话题才能拖拽文件。"); // 调试信息
        ...
        if (files && files.length > 0) {
            ...
            if (failedFiles.length > 0) {
                failedFiles.forEach(f => {
                    console.error(`处理拖拽的文件 ${f.name} 失败: ${f.error}`); // 调试信息
            ...
            try {
                //console.log('[InputEnhancer] Calling localtauriAPI.handleFileDrop with:', agentId, topicId, successfulFiles.map(f => ({ name: f.name, type: f.type, size: f.size, data: f.data ? `[Buffer, length: ${f.data.length}]` : 'N/A' })));
                // Pass the actual buffers to main process
                const results = await localtauriAPI.handleFileDrop(agentId, topicId, successfulFiles);
                ...
                if (results && results.length > 0) {
                    results.forEach(result => {
                        if (result.success && result.attachment) {
                            ...
                            console.error(`处理拖拽的文件 ${result.name || '未知文件'} 失败: ${result.error}`); // 调试信息
                        }
                    });
                    updateAttachmentPreviewRef();
                }
            } catch (err) {
                console.error('[InputEnhancer] Error calling localtauriAPI.handleFileDrop IPC:', err);
                console.error('处理拖拽的文件时发生意外错误。'); // 调试信息
            }
        } else {
    ...
    messageInput.addEventListener('paste', async (event) => {
        ...
        if (hasFile) {
            event.preventDefault();
            if (!agentId || !topicId) {
                console.error("请先选择一个Agent和话题才能粘贴文件。"); // 调试信息
                return;
            }
            // 寻找第一个文件并处理
            for (let i = 0; i < items.length; i++) {
            ...
            // 如果有截图，也算文件，上面的逻辑会处理
            const imageData = await localtauriAPI.readImageFromClipboard();
        ...
        if (pastedText && pastedText.length > LONG_TEXT_THRESHOLD) {
            event.preventDefault();
            if (!agentId || !topicId) {
                console.error("请先选择一个Agent和话题才能粘贴长文本。"); // 调试信息
                return;
            }
    ...
    localtauriAPI.onAddFileToInput(async (filePath) => {
        //console.log(`[InputEnhancer] Received shared file path: ${filePath}`);
        const agentId = currentAgentIdRef();
        const topicId = currentTopicIdRef();

        if (!agentId || !topicId) {
            console.error("请先选择一个Agent和话题才能分享文件。"); // 调试信息
            return;
        }
        ...
        try {
            ...
            const results = await localtauriAPI.handleFileDrop(agentId, topicId, [{ path: filePath, name: fileName }]);
            if (results && results.length > 0 && results[0].success && results[0].attachment) {
                ...
                console.error(`附加分享的文件失败: ${errorMsg}`); // 调试信息
            }
        } catch (err) {
            console.error('[InputEnhancer] Error attaching shared file:', err);
            console.error('附加分享的文件时发生意外错误。'); // 调试信息
        }
    });
    ...
    messageInput.addEventListener('input', async () => {
        ...
        if (atMatch) {
            ...
            const notes = await localtauriAPI.searchNotes(query);
            if (notes.length > 0) {
    ...
    async function selectNoteSuggestion(note) {
        ...
        const topicId = currentTopicIdRef();
        if (!agentId || !topicId) {
            console.error("请先选择一个Agent和话题才能附加笔记。"); // 调试信息
            return;
        }
        ...
        try {
            const results = await localtauriAPI.handleFileDrop(agentId, topicId, [{
            ...
            if (results && results.length > 0 && results[0].success && results[0].attachment) {
                ...
                const errorMsg = results && results.length > 0 && results[0].error ? results[0].error : '未知错误';
                console.error(`附加笔记 "${note.name}" 失败: ${errorMsg}`); // 调试信息
            }
        } catch (err) {
            console.error('[InputEnhancer] Error attaching note file:', err);
            console.error(`附加笔记 "${note.name}" 时发生意外错误。`); // 调试信息
        }
    }
}
...
async function handlePastedFile(file, agentId, topicId) {
    ...
    reader.onload = async (e_reader) => {
        ...
        if (fileBuffer.length > 0 || file.size === 0) {
            try {
                const results = await window.tauriAPI.handleFileDrop(agentId, topicId, [{
                    name: file.name,
                    type: file.type || 'application/octet-stream',
                    data: fileBuffer,
                    size: file.size
                ...
                if (results && results.length > 0 && results[0].success && results[0].attachment) {
                    ...
                    const errorMsg = results && results.length > 0 && results[0].error ? results[0].error : '未知错误';
                    console.error(`粘贴文件 "${file.name}" 失败: ${errorMsg}`); // 调试信息
                }
            } catch (err_ipc) {
                console.error(`粘贴文件 "${file.name}" 时发生 IPC 错误。`); // 调试信息
            }
        } else {
            console.error(`无法读取粘贴的文件 "${file.name}" 的内容。`); // 调试信息
        }
    };
    reader.onerror = () => console.error(`读取粘贴的文件 "${file.name}" 失败。`); // 调试信息
    reader.readAsArrayBuffer(file);
}

async function handlePastedImageData(imageData, agentId, topicId) {
    try {
        const result = await window.tauriAPI.handleFilePaste(agentId, topicId, {
            type: 'base64',
            data: imageData.data,
        ...
        if (result.success && result.attachment) {
            ...
            console.error(`无法从剪贴板粘贴图片: ${result.error || '截图处理失败'}`); // 调试信息
        }
    } catch (e) {
        console.error(`粘贴截图时发生错误: ${e.message}`); // 调试信息
    }
}

async function handleLongTextPaste(pastedText, agentId, topicId) {
    try {
        const result = await window.tauriAPI.handleTextPasteAsFile(agentId, topicId, pastedText);
        if (result.success && result.attachment) {
            const att = result.attachment;
            const currentFiles = attachedFilesRef.get();
            currentFiles.push({
            ...
            console.error(`长文本转存为 .txt 文件失败: ${result.error || '未知错误'}`); // 调试信息
        }
    } catch (err) {
        console.error('长文本转存时发生意外错误。'); // 调试信息
    }
}
